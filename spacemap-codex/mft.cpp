...
static std::wstring strip_extended_prefix(const std::wstring& path) {
...
static bool mft_debug_enabled() {
    return std::getenv("SPACEMAP_MFT_DEBUG") != NULL;
}

static bool mft_fail(const char* reason) {
    DWORD err = GetLastError();
    if (mft_debug_enabled()) {
        std::cerr << "MFT failed: " << reason << " (GetLastError=" << err << ")\n";
    }
    return false;
}

static bool read_volume_at(HANDLE vol, LONGLONG offset, BYTE* buf, DWORD bytes_to_read, DWORD& bytes_read) {
    LARGE_INTEGER li;
    li.QuadPart = offset;
    ...
    return ReadFile(vol, buf, bytes_to_read, &bytes_read, NULL) != 0;
}
...
static bool build_dir_path(DWORDLONG ref,
                           std::vector<std::pair<DWORDLONG, MftFileEntry>>& entries,
                           const std::unordered_map<DWORDLONG, int>& dir_idx,
                           std::unordered_map<DWORDLONG, std::wstring>& path_cache,
                           const std::wstring& vol_root,
                           DWORDLONG volume_root_ref,
                           bool has_volume_root_ref,
                           int depth,
                           std::wstring& out) {
    if (depth > 256) return false;

    std::unordered_map<DWORDLONG, std::wstring>::iterator cached = path_cache.find(ref);
    if (cached != path_cache.end()) {
        out = cached->second;
        return true;
    }

    auto it = find_entry(entries, ref);
    if (it == entries.end() || !it->second.is_dir) return false;

    bool is_volume_root = (has_volume_root_ref && ref == volume_root_ref) ||
                          it->second.parent_ref == ref;
    if (is_volume_root) {
        out = vol_root;
        path_cache[ref] = out;
        return true;
    ...
    std::unordered_map<DWORDLONG, int>::const_iterator parent_it = dir_idx.find(it->second.parent_ref);
    if (parent_it == dir_idx.end()) return false;

    std::wstring parent_path;
    if (!build_dir_path(it->second.parent_ref, entries, dir_idx, path_cache,
                        vol_root, volume_root_ref, has_volume_root_ref,
                        depth + 1, parent_path)) {
        return false;
    }

    out = join_path(parent_path, it->second.name.c_str());
    path_cache[ref] = out;
    return true;
}
...
bool get_dir_size_mft(const std::wstring& root, int top_n, MftScanResult& out) {
    ...
    std::wstring scan_root = strip_extended_prefix(root);
    std::wstring vol_root;
    size_t colon = scan_root.find(L':');
    if (colon != std::wstring::npos) {
        vol_root = scan_root.substr(0, colon + 1) + L"\\";
    } else {
        return mft_fail("no drive letter");
    }

    // Fix ⑥: Bulk read MFT — read boot sector to find MFT location
    std::wstring vol_dev = L"\\\\.\\" + vol_root.substr(0, 2);
    DWORD volume_access = FILE_READ_DATA | FILE_READ_ATTRIBUTES;
    HANDLE vol = CreateFileW(vol_dev.c_str(), volume_access,
                             FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
                             NULL, OPEN_EXISTING, 0, NULL);
    if (vol == INVALID_HANDLE_VALUE) return mft_fail("open raw volume");

    // Read boot sector (first 512 bytes)
    BYTE boot_buf[512];
    DWORD boot_read = 0;
    if (!read_volume_at(vol, 0, boot_buf, 512, boot_read) || boot_read < 512) {
        CloseHandle(vol);
        return mft_fail("read boot sector");
    }

    const NtfsBootSector* boot = (const NtfsBootSector*)boot_buf;
    if (memcmp(boot->oem, "NTFS", 4) != 0) {
        CloseHandle(vol);
        return mft_fail("not ntfs boot sector");
    }
    ...
    HANDLE vol_enum = CreateFileW(vol_dev.c_str(), volume_access,
                                  FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
                                  NULL, OPEN_EXISTING, 0, NULL);
    if (vol_enum == INVALID_HANDLE_VALUE) {
        CloseHandle(vol);
        return mft_fail("open volume for usn enum");
    }
    ...
    while (!g_interrupted.load()) {
        if (!DeviceIoControl(vol_enum, FSCTL_ENUM_USN_DATA, &med, sizeof(med),
            ...
            if (mft_debug_enabled()) {
                std::cerr << "MFT enum stopped: GetLastError=" << GetLastError()
    ...
    if (entries.empty()) {
        CloseHandle(vol);
        return mft_fail("no usn entries");
    }
    ...
    if (mft_data_length <= BULK_THRESHOLD) {
        ...
        while (mft_offset < mft_end && !g_interrupted.load()) {
            ...
            DWORD bytes_read = 0;
            if (!read_volume_at(vol, mft_offset, chunk_buf, to_read, bytes_read) ||
                bytes_read < (DWORD)mft_rec_size) {
                break;
            }
        ...
        for (auto it = entries.begin(); it != entries.end(); ++it) {
            ...
            DWORD bytes_read = 0;
            if (read_volume_at(vol, offset, rec_buf, mft_rec_size, bytes_read) && bytes_read >= 48) {
    ...
    DWORDLONG volume_root_ref = 0;
    bool has_volume_root_ref = false;
    for (auto it = entries.begin(); it != entries.end(); ++it) {
        if (it->second.is_dir && it->second.parent_ref == it->first) {
            volume_root_ref = it->first;
            has_volume_root_ref = true;
            break;
        }
    }
    ...
    std::wstring norm_root = scan_root;
    ...
    for (int i = 0; i < nd; i++) {
        std::wstring ignored;
        build_dir_path(dir_refs[i], entries, dir_idx, path_cache,
                       vol_root, volume_root_ref, has_volume_root_ref, 0, ignored);
    }
    ...
    if (root_idx < 0) {
        if (mft_debug_enabled()) {
            std::cerr << "MFT failed: root path not found, norm_root=" << ws2s(norm_root)
                      << ", dirs=" << nd << ", entries=" << entries.size() << "\n";
        }
        return false;
    }
    ...
    long long direct_root_size = 0;
    unsigned long long direct_root_files = 0;
    long long sum_size = 0;
    unsigned long long sum_files = 0;
    unsigned long long sum_dirs = 1;
    ...
    for (size_t ci = 0; ci < children[root_idx].size(); ci++) {
        ...
        if (mft_should_skip(name, eit->second.file_attr)) {
            out.scan.stats.skipped++;
            continue;
        }
    ...
    for (auto it = entries.begin(); it != entries.end(); ++it) {
        if (it->second.is_dir || it->second.size <= 0) continue;
        if (it->second.file_attr & FILE_ATTRIBUTE_REPARSE_POINT) continue;
        if (it->second.parent_ref != dir_refs[root_idx]) continue;
        direct_root_files++;
        direct_root_size += it->second.size;
    }

    out.scan.stats.dirs = sum_dirs;
    out.scan.stats.files = sum_files + direct_root_files;
    out.scan.stats.size = sum_size + direct_root_size;
