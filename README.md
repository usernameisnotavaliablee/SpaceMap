...
|-|-|-|-|-|
|价格|免费开源（MIT）|免费|免费（功能受限）|免费|
|启动速度|即时|慢（需扫描）|较快|即时|
|颜色/图表|渐变 + █ 柱状图|有，但 GUI|有，但 GUI|无|
|交互模式|有（TUI）|有（GUI）|有（GUI）|无|
|命令行|原生|需要 GUI|需要 GUI|原生|
|依赖|无（单文件 exe）|需安装|需安装|需安装|
|性能|多线程 + MFT 直读，2-5s/NTFS|慢|MFT 直读，快|单线程|
|文件类型分类|✓|✓|✗（专业版才有）|✗|
|脚本/自动化|✓（CLI 原生）|✗|✗|✓|
